...
function buildImages(apodData) {
    const html = apodData.map(apod => /*html*/`
        <div class="image col-12 mb-3">
            <img src="${apod.url}" alt="${apod.title}" class="img-fluid mx-auto d-block" onclick="openHdImage('${apod.hdurl}')">
            <h2 class="text-center p-3">${apod.title}</h2>
            <p class="text-center p-3">${apod.explanation}</p>
            <button class="save btn btn-primary mx-auto d-block"
                data-id="${apod.date}"
